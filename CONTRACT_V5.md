...
### `GET /lines`
```json
{ "lines": [ { "line": "경부선", "stations": ["서울","영등포","수원","대전","동대구","부산"] } ] }
```
...
### `GET /vulnerability/stations?line=경부선&alert_type=대설&alert_level=경보`
```json
{
  "line": "경부선", "alert_type": "대설", "alert_level": "경보",
  "stations": [
    { "station": "대전", "avg_delay": 22.4, "delay_rate": 0.61, "stop_rate": 0.05,
      "delta_delay": 18.1, "sample_n": 44 }
  ]
}
```
...
{
  "line": "경부선",
  "nodes": [ { "station": "대전", "lat": 36.33, "lon": 127.43, "vuln": 0.82 } ],
  "edges": [ { "from": "대전", "to": "김천(구미)", "vuln": 0.78 } ]
}
```
...
```json
{ "station": "대전",
  "by_alert": [ { "alert_type": "대설", "alert_level": "경보", "avg_delay": 22.4, "sample_n": 44 } ],
  "cases": [ { "date": "2026-07-04", "alert_type": "호우", "delay_min": 31 } ] }
```

### `GET /checklist?line=경부선` — 우선 점검 대상 Top-N
```json
{ "line": "경부선",
  "items": [ { "rank": 1, "target": "대전→김천(구미) 구간", "reason": "호우 경보 시 평균 +14.6분",
               "avg_delay_incr": 14.6, "sample_n": 37 } ] }
```
...
```json
{
  ...
  "active": [
    { "region_name": "대전", "alert_type": "호우", "alert_level": "경보",
      ...
      "affected": [
        { "type": "segment", "from": "대전", "to": "김천(구미)", "vuln_rank": 1,
...
- 필터 키·허용값을 고정: `alert_type`(대설/호우/폭염/강풍/태풍/한파), `alert_level`(주의보/경보), `train_type`(all/KTX/무궁화/새마을), `line`.
