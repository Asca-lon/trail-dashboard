...
mock 데이터 연결은 완료되었습니다.

다음 구현 단계에서는 상세보기 버튼 동작 또는 실제 API 전환 계획을 설계합니다.
...
5. API 응답 데이터를 View Model로 변환하면 어떤 장점이 있나요?
6. `fetch()`를 사용할 때 `file://` 실행보다 로컬 서버 실행이 안정적인 이유는 무엇인가요?
7. `Promise.all`로 여러 데이터를 동시에 요청하면 어떤 장점과 주의점이 있나요?
8. mock에 없는 데이터를 화면에 표시해야 할 때 `-`로 표시하는 방식의 장단점은 무엇인가요?
9. 노선형 위험도 지도와 실제 지도 기반 위험도 지도의 차이는 무엇인가요?
10. TOP 5 랭킹의 정렬 기준을 프론트엔드에 둘 때와 백엔드에 둘 때의 차이는 무엇인가요?
11. 운영 판단에 영향을 주는 대응 권고를 프론트엔드에서 추론하면 어떤 위험이 있나요?
12. 브라우저 자동화 도구로 시각 검수를 자동화하면 어떤 장점이 있나요?
