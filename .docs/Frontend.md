...
구간 상세 화면의 상단 탭 목록에서 `과거 사례` 탭을 제거했습니다.

# 구현 목적

현재 사용하지 않는 과거 사례 진입점을 화면에서 노출하지 않고 구간 상세 탐색 구조를 단순화합니다.

# 구현 내용

- `route-tab-history` 버튼을 제거했습니다.
- `요약`, `특보별 분석` 탭은 기존 동작을 유지합니다.
- 앞선 작업에서 제거한 구간 거리 항목이 다시 노출되지 않는지 확인했습니다.

# 코드 설명

탭 버튼 마크업만 제거해 사용자의 접근 경로를 없앴습니다. 숨겨진 과거 사례 패널과 관련 JavaScript는 사용자 요청 범위를 넘지 않도록 이번 작업에서 변경하지 않았습니다.
...
- 과거 사례 패널 코드는 남아 있지만 화면에서 해당 탭으로 전환할 수 없습니다.
- `mock/` 폴더는 수정하지 않았습니다.
...
- 필요 시 접근할 수 없게 된 과거 사례 패널과 관련 코드 정리
- 전체 회귀 테스트
...
사용하지 않는 구간 상세 메뉴를 제거해 화면 구조를 간결하게 만들었습니다.

## 변경 사항

- 탭 목록을 요약과 특보별 분석 두 개로 축소

## 새롭게 배운 개념

- Navigation Surface Removal
- Scope-limited Change

## 실무에서는

접근 경로를 제거한 기능이 다시 사용되지 않는다면 후속 작업으로 관련 DOM, JavaScript, CSS까지 정리해 기술 부채를 줄입니다.

## 개선 가능한 부분

- 과거 사례 패널 및 관련 렌더링 코드 완전 제거
- 탭 전환 자동화 테스트 추가
...
1. 탭 버튼만 제거하고 패널 코드를 유지할 때의 장단점은 무엇일까요?
2. 사용하지 않는 UI 진입점을 제거하면 사용자 경험이 어떻게 개선될까요?
3. 후속 코드 정리가 필요한지 판단하는 기준은 무엇일까요?
...
구간 상세 내부 탭 구성 변경이므로 README에는 별도 항목을 추가하지 않았습니다.

## 추천 Commit Message

`chore: 구간 상세 과거 사례 탭 제거`
