...
- 대시보드 요약 카드의 비교 문구를 제거했습니다.
- 역 상세 차트의 주말/공휴일 범례를 제거하고 평일 범례를 `실제`로 변경했습니다.
- 노선 상세 차트의 예측 범례를 제거했습니다. 차트 데이터 선 자체는 유지합니다.
...
`refactor: 대시보드와 상세 화면의 보조 정보 간소화`

## Timestamp

2026-07-16 11:08:49 (KST)
...
메인 대시보드, 역 상세, 구간 상세 페이지의 전역 헤더에 공통 AI 아이콘을 추가하고, 현재 화면을 벗어나지 않고 질문할 수 있는 오른쪽 채팅 드로어를 구현했습니다.

이번 단계는 프론트엔드 Mock UI만 다룹니다. 백엔드 API, RAG 출처, 대화 저장과 스트리밍 응답은 연결하지 않았습니다.

# 구현 목적

- 사용자가 분석 화면을 유지한 채 AI 질문을 시작할 수 있게 합니다.
- 세 페이지에서 같은 채팅 경험을 제공하면서 페이지별 컨텍스트와 추천 질문을 분리합니다.
- 향후 `/rag/ask` 연결 시 공통 JavaScript 한 곳만 확장할 수 있는 구조를 마련합니다.
- 키보드와 보조 기술 사용자가 채팅 UI의 상태 변화를 이해할 수 있게 합니다.

# 구현 내용

- 세 페이지 전역 헤더 오른쪽에 `40 × 40px` AI 트리거 버튼을 추가했습니다.
- 데스크톱 `400px`, 태블릿 `360px`, 모바일 전체 화면 드로어를 구현했습니다.
- 페이지별 컨텍스트를 현재 DOM에서 읽어 자동 갱신합니다.
- 페이지별 추천 질문 세 개를 제공합니다.
- 빈 화면, 사용자·AI 메시지, 로딩, 오류, 데이터 없음, 추천 질문 선택 상태를 구현했습니다.
- `Enter` 전송, `Shift + Enter` 줄바꿈, `ESC` 닫기를 지원합니다.
- 최소화·닫기 후 AI 트리거로 포커스가 돌아옵니다.
- 입력 길이를 500자로 제한하고 실시간 글자 수와 전송 가능 상태를 표시합니다.
- 애니메이션 감소 설정에서는 드로어 전환과 로딩 점 애니메이션을 제거합니다.

# 코드 설명

## 왜 필요한가

페이지마다 채팅 마크업과 동작을 복제하면 추천 질문이나 API 계약을 바꿀 때 세 파일을 함께 수정해야 합니다. `ai-chat.js`가 공통 UI를 한 번 생성하고 페이지 설정만 구분하도록 만들어 중복과 수정 누락 가능성을 줄였습니다.

## 어떤 원리인가

1. 현재 URL의 HTML 파일명을 기준으로 페이지 설정을 선택합니다.
2. 공통 헤더에 AI 트리거를 추가하고 `body`에 시맨틱 채팅 드로어를 생성합니다.
3. 역 상세는 현재 역명, 구간 상세는 출발역과 도착역 DOM을 읽어 컨텍스트를 표시합니다.
4. 기존 페이지 JavaScript가 대상 이름을 갱신하면 `MutationObserver`가 변화를 감지해 컨텍스트도 갱신합니다.
5. 질문 전송 시 사용자 메시지와 로딩 상태를 순서대로 표시한 후 페이지별 Mock 답변을 렌더링합니다.

## 실행 흐름

```text
AI 아이콘 클릭
→ 드로어 열기 및 현재 화면 컨텍스트 갱신
→ 추천 질문 선택 또는 직접 입력
→ 사용자 메시지 표시
→ Mock 로딩 상태
→ 예시 답변 / 오류 / 데이터 없음 상태
...
- 세 페이지가 하나의 채팅 로직과 스타일을 공유합니다.
- 페이지 데이터 렌더링과 채팅 UI가 느슨하게 연결되어 기존 화면 회귀 위험이 작습니다.
- API 연결 전에도 모든 주요 UI 상태와 키보드 흐름을 검증할 수 있습니다.
- SVG 아이콘을 기존 에셋 방식으로 관리해 새 라이브러리가 필요하지 않습니다.

## 단점

- 현재 답변은 실제 화면 데이터를 분석한 결과가 아닌 예시 문구입니다.
- 새로고침하면 대화가 초기화됩니다.
- 서버 오류와 데이터 없음 상태는 실제 응답 코드가 아니라 질문의 테스트 키워드로 재현합니다.

## 다른 구현 방법

- 세 HTML에 채팅 마크업을 직접 작성할 수 있지만 중복 관리 비용이 큽니다.
- Web Component로 캡슐화할 수 있지만 현재 프로젝트 규모에서는 공통 JavaScript가 더 단순합니다.
- React 전환 후 `lucide-react`와 상태 관리 라이브러리를 사용할 수 있지만 이번 작업에는 빌드 환경과 의존성 추가가 필요하지 않습니다.

## 실무에서는

실제 서비스에서는 Mock 응답 부분을 `/rag/ask` 호출로 교체하고 요청 취소, 네트워크 타임아웃, 출처 표시, 개인정보 필터링, 관측성 및 대화 보존 정책을 함께 설계합니다. LLM 본문에 포함된 숫자를 다시 파싱하지 않고 서버 응답의 구조화된 `targets[].metrics`를 표시해야 합니다.
...
|---|---|---|---|---|
| `POST` | `/rag/ask` | `question`, 선택적 `context` | `answer`, `targets`, `sources`, `limitations`, `resolution` | HTTP 오류 및 오류 본문 |

현재 Mock 테스트 키워드:

- 일반 질문: 예시 AI 답변
- `오류` 포함 질문: 오류 상태와 다시 시도 버튼
- `데이터 없음` 포함 질문: 데이터 없음 상태

# Database

Database와 Schema 변경은 없습니다. ERD 변경도 필요하지 않습니다.

# 주의사항

- 채팅 답변은 실제 RAG 결과가 아니며 의사결정 근거로 사용할 수 없습니다.
- 드로어에는 오버레이를 사용하지 않아 배경 화면을 계속 확인할 수 있습니다.
- 드로어는 화면에 고정하기 위해 `position: fixed`를 사용하지만, 신규 AI UI에는 `position: absolute`를 사용하지 않았습니다.
- 숨김 `label`은 기존 공통 접근성 클래스인 `.sr-only`를 재사용합니다.
- 헤드리스 Edge가 현재 실행 환경에서 스크린샷을 만들지 못해 자동 시각 비교는 수행하지 못했습니다.
...
- 세 페이지 공통 AI 트리거 및 채팅 드로어 추가
- 페이지별 컨텍스트와 추천 질문 추가
- 7개 Mock UI 상태와 키보드 상호작용 추가
- 데스크톱·태블릿·모바일 반응형 스타일 추가
- README에 Mock 상태 확인 방법 추가

# 다음 작업

- 실제 브라우저에서 1280px, 768px, 375px 시각 검수
- `/rag/ask` API 연결에 대한 별도 설계 및 사용자 동의
- 실제 응답의 출처·제약사항·구조화 지표 표시 설계

# 작업 요약

## 완료한 내용

- 공통 AI 아이콘 및 채팅 드로어 구현
- 페이지별 현재 화면 컨텍스트 자동 반영
- Mock 질문·로딩·답변·오류·데이터 없음 상태 구현
- 접근성 및 반응형 구현
- 정적 검사와 HTTP 경로 검사
...
- `frontend/style.css`
- `frontend/ai-chat.js`
- `frontend/assets/icons/bot.svg`
- `frontend/assets/icons/minus.svg`
- `frontend/assets/icons/x.svg`
- `frontend/assets/icons/send-horizontal.svg`
- `frontend/assets/icons/database-x.svg`
- `.docs/Frontend_Common.md`
- `README.md`

## 구현 이유

현재 분석 화면과 연결된 AI 진입점을 일관되게 제공하고, 실제 RAG API 연결 전에 반응형 UI와 전체 상호작용 상태를 검증하기 위해 구현했습니다.

## 변경 사항

- 백엔드·Database 변경 없음
- 신규 외부 라이브러리 없음
- 기존 폴더 구조와 페이지별 데이터 렌더링 유지
- 공통 AI 채팅 JavaScript와 SVG 에셋 추가

## 새롭게 배운 개념

- 비모달 채팅 드로어
- `aria-live`와 `aria-busy`
- `MutationObserver` 기반 컨텍스트 동기화
- `prefers-reduced-motion`

## 실무에서는

프론트엔드 상태를 `idle`, `loading`, `success`, `error`, `no-data`처럼 명시적으로 관리하고, API 요청마다 취소 가능한 `AbortController`를 사용합니다. 접근성 회귀는 키보드 E2E 테스트와 스크린 리더 점검으로 함께 검증합니다.

## 개선 가능한 부분

- 실제 API 연결 후 응답 상태를 Mock 키워드 방식에서 HTTP 상태 기반으로 교체
- 대화 초기화 버튼과 출처 UI 설계
- Playwright 기반 반응형·키보드·시각 회귀 테스트 추가

## 다음 작업

- 실제 RAG API 연결 범위와 오류 계약 설계
...
1. 세 페이지의 채팅 UI를 공통 JavaScript로 관리하면 어떤 유지보수 이점이 있을까요?
2. 동적으로 추가되는 메시지 영역에 `aria-live`와 `aria-busy`가 필요한 이유는 무엇일까요?
3. 실제 `/rag/ask` 연결 시 LLM 본문의 숫자 대신 `targets[].metrics`를 사용해야 하는 이유는 무엇일까요?
...
사용자에게 노출되는 AI 채팅 기능과 Mock 상태 확인 방법이 추가되어 README를 업데이트했습니다.

## 추천 Commit Message

`feat: 페이지별 AI 채팅 드로어 Mock UI 추가`

## Timestamp

2026-07-20 17:52:14 (KST)
