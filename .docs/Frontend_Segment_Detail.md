...
구간 상세의 `현재 위험도`와 `현재 발효 특보`가 서로 다른 의미에 맞는 색상 기준을 사용하도록 개선했습니다. 현재 위험도는 기존 취약도 등급 색상을 유지하고, 현재 발효 특보 박스는 기상특보의 `alert_level`을 기준으로 표시합니다.

# 구현 목적

구간 취약도가 `관심`이더라도 실제 발효 특보가 `경보`이면 특보 박스는 빨간색이어야 합니다. 취약도와 기상특보 등급을 분리해 사용자가 현재 특보의 심각도를 정확히 인식하도록 합니다.

# 구현 내용

- `경보` 전용 `route-alert-card__box--emergency` 상태 추가
- `주의보` 전용 `route-alert-card__box--advisory` 상태 추가
- 특보 없음 또는 알 수 없는 등급은 `route-alert-card__box--none` 적용
- 여러 특보가 동시에 연결되면 `경보`를 `주의보`보다 우선 적용
- 세 탭에 반복 배치된 현재 발효 특보 박스에 동일한 규칙 적용
- 현재 위험도 카드의 `risk_level` 색상 로직은 변경하지 않음

# 코드 설명

`renderActiveAlerts()`는 선택 구간에 연결된 현재 특보를 찾은 뒤 `getActiveAlertCardStyle()`에 전달합니다. 이 함수는 `alert_level === "경보"`를 먼저 확인하고, 경보가 없을 때 `주의보`를 확인합니다. 어느 등급도 없으면 `none`을 반환합니다. CSS 사용자 정의 속성으로 각 상태의 강조색, 배경색, 테두리색, 아이콘 필터를 한 modifier에 모았습니다.

장점은 취약도와 특보 등급의 의미가 섞이지 않고, 경보의 시각적 우선순위가 보장된다는 점입니다. 단점은 여러 특보가 한 박스에 함께 표시될 때 박스 전체가 가장 높은 등급의 색상을 사용한다는 점입니다. 다른 구현으로 특보별 박스를 분리해 각 등급 색상을 적용할 수 있습니다. 실무에서는 취약도 토큰과 재난특보 토큰을 별도 디자인 토큰으로 관리합니다.
...
색상만으로 상태를 전달하지 않도록 `호우 경보`, `폭염 주의보` 등의 기존 텍스트를 유지합니다. 예상하지 못한 특보 등급은 과도한 경고색을 적용하지 않고 정보 없음 색상으로 처리합니다.

# 변경 이력

2026-07-22 - 현재 발효 특보 박스를 특보 등급별 색상으로 분리

# 다음 작업

특보별 박스를 분리해 복수 특보 각각의 등급 색상을 표시하는 방식 검토

# 작업 요약

## 완료한 내용

- 경보 빨간색, 주의보 노란색, 특보 없음 회색 적용
- 여러 특보 중 가장 높은 등급 우선 처리
- 현재 위험도 색상 로직과 현재 발효 특보 색상 로직 분리
- 세 탭의 현재 발효 특보 박스에 동시 적용
...
현재 발효 특보의 발표 등급을 구간 취약도와 혼동하지 않도록 했습니다.

## 변경 사항

- 취약도 기반 4색 박스 → 특보 등급 기반 경보·주의보·없음 박스

## 새롭게 배운 개념

- 서로 다른 도메인 상태의 시각 토큰 분리
- 복수 특보의 최상위 심각도 선택

## 실무에서는

취약도와 재난특보 등급은 비슷한 색상을 쓰더라도 별도의 상태 모델과 CSS 클래스로 관리합니다. 색각 이상 사용자를 위해 색상과 함께 특보 종류·등급 텍스트를 제공합니다.
...
- 경보·주의보·복수 특보 시각 회귀 테스트 추가

## 복습 문제

1. 현재 위험도와 현재 발효 특보가 서로 다른 색상 기준을 사용해야 하는 이유는 무엇일까요?
2. 경보와 주의보가 동시에 발효되면 경보 색상을 우선해야 하는 이유는 무엇일까요?
3. 예상하지 못한 특보 등급을 회색으로 처리하면 어떤 장단점이 있을까요?

## 오늘 배운 내용

- 취약도 등급과 기상특보 등급의 표현 책임 분리
...
`fix: 구간 현재 특보 색상을 발효 등급 기준으로 분리`

## Change Log

2026-07-22 - route-alert-card__box 특보 등급별 색상으로 변경

## Timestamp

2026-07-22 10:34:32 (KST)
...
구간 상세 페이지의 지연 증가량 추이에서 마지막 데이터 막대의 범례를 `오늘`에서 `어제`로 변경했습니다.

# 구현 목적

차트가 제공하는 최근 확정 데이터의 시점을 화면에 정확히 표현합니다.

# 구현 내용

- 범례 문구를 `오늘`에서 `어제`로 변경했습니다.
- 범례 modifier를 `--today`에서 `--yesterday`로 변경했습니다.
- 마지막 막대 modifier와 JavaScript 변수명도 `yesterday` 의미로 통일했습니다.
- 기존 강조 색상과 마지막 막대 선택 동작은 유지했습니다.

# 코드 설명

`createDelayTrendBar()`는 배열의 마지막 항목을 어제 데이터로 간주해 `route-bar-chart__item--yesterday` 클래스를 추가합니다. CSS는 해당 클래스에 기존과 동일한 빨간색 강조를 적용합니다.

장점은 화면 문구와 코드의 의미가 일치한다는 점입니다. 단점은 API 데이터의 마지막 항목이 항상 어제라는 전제가 필요하다는 점입니다. 다른 방법으로는 날짜를 현재 KST 날짜와 비교해 어제 항목을 찾을 수 있습니다. 실무에서는 API가 데이터 기준일을 명시하고 프론트엔드는 그 날짜를 기준으로 범례와 강조 상태를 결정합니다.
...
- 현재는 데이터 배열의 마지막 항목을 어제로 간주합니다.
- 색상과 막대 강조 동작은 변경하지 않았습니다.

# 변경 이력

2026-07-20 - 지연 증가량 추이의 오늘 범례를 어제로 변경

# 다음 작업

- 데이터 기준일을 API 응답으로 명시하는 방안 검토
...
- 범례 문구 `어제` 적용
- HTML·CSS modifier를 `yesterday`로 통일
- JavaScript 변수와 동적 클래스명을 `yesterday`로 통일
- JavaScript 문법 및 이전 명칭 잔존 여부 검증
...
차트의 최신 확정 데이터가 어제 기준임을 사용자와 개발자 모두 명확하게 이해하도록 변경했습니다.
...
화면 라벨만 변경하지 않고 관련 변수와 클래스도 같은 의미로 정리해 코드 검색과 유지보수가 쉬운 상태를 유지합니다.

## 개선 가능한 부분

- KST 날짜를 기준으로 어제 데이터를 직접 식별
- API 응답에 데이터 기준일 추가
- 날짜별 강조 로직 단위 테스트 추가
...
1. `node --check frontend/route-detail.js`로 문법을 검사합니다.
2. 세 프론트엔드 파일에서 `today` 관련 이전 참조가 없는지 검색합니다.
3. `yesterday` 클래스와 변수 및 `어제` 문구가 모두 존재하는지 확인합니다.

## 예상 결과

- 범례에 `어제`가 표시됩니다.
- 마지막 막대가 기존과 같은 빨간색으로 강조됩니다.
- `today` 관련 이전 참조가 남지 않습니다.

## 실패 시 확인 사항

- HTML 범례 클래스와 CSS 선택자가 일치하는지 확인합니다.
- JavaScript가 마지막 막대에 `route-bar-chart__item--yesterday`를 추가하는지 확인합니다.

## 복습 문제

1. 화면 문구를 변경할 때 관련 클래스와 변수명도 함께 바꾸는 이유는 무엇인가요?
2. 배열의 마지막 항목을 항상 어제로 간주할 때 발생할 수 있는 문제는 무엇인가요?
3. 시계열 API가 데이터 기준일을 제공하면 어떤 장점이 있나요?
...
`fix: 구간 상세 지연 증가량 범례를 어제로 변경`

## Change Log

2026-07-20

- 지연 증가량 추이 범례를 어제로 변경
- 관련 HTML·CSS·JavaScript 명칭 정리
- 구간 상세 문서 업데이트

## Timestamp

2026-07-20 09:58:32 (KST)
