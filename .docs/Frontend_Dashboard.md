...
대시보드 위험도 현황 카드와 오른쪽 TOP 5 카드 묶음의 세로 높이를 통일했습니다.

# 구현 목적

좌우 카드 하단을 정렬해 대시보드 그리드의 시각적 균형을 유지합니다.
...
- TOP 5 영역 높이: 568px
- 각 TOP 5 카드 높이: 276px
- 카드 간격 16px을 포함해 `276 + 16 + 276 = 568px`로 정렬
- 1024px 이하에서는 위험도 현황 카드 높이를 콘텐츠 기반 `auto`로 전환

# 코드 설명

데스크톱에서는 좌우 열의 총 높이를 동일하게 고정하고, 세로 적층으로 바뀌는 반응형 구간에서는 고정 높이를 해제했습니다. 장점은 정렬이 명확하다는 점이고, 단점은 카드 내용이 크게 늘면 높이 재검토가 필요하다는 점입니다. 실무에서는 CSS Grid의 동일 행 배치와 시각 회귀 테스트를 함께 사용합니다.
...
TOP 5 데이터가 5개를 초과하지 않는 현재 계약을 기준으로 합니다.

# 변경 이력

2026-07-13 - 위험도 현황과 TOP 5 카드 세로 정렬

# 다음 작업

브라우저에서 1024px 경계 시각 검수
...
카드 하단 정렬과 정보 영역의 균형을 개선했습니다.

## 변경 사항

- 252px 순위 카드 두 개를 각각 276px로 확대
...
카드 내부 데이터 길이와 확대 글꼴 환경까지 포함해 높이를 검증합니다.

## 개선 가능한 부분

- CSS Grid 기반 자동 동일 높이 적용
...
1. 두 카드와 16px 간격의 합이 568px이 되려면 각 카드 높이는 얼마일까요?
2. 모바일에서 고정 높이를 해제한 이유는 무엇일까요?
3. 고정 높이 방식의 단점은 무엇일까요?
...
표준 Python 정적 서버의 루트 URL에서 기존 API/Mock 콘솔 대신 실제 메인 대시보드가 열리도록 진입점을 변경했습니다.

# 구현 목적

사용자가 `http://127.0.0.1:8765/`에 접속했을 때 별도의 하위 경로를 입력하지 않고 `frontend/dashboard.html`로 이동하게 합니다.

# 구현 내용

- 루트 `index.html`을 대시보드 전용 진입 페이지로 교체
- JavaScript `location.replace()`를 이용한 즉시 이동
- JavaScript 비활성 환경을 위한 HTML `meta refresh`와 수동 링크 제공
- 쿼리 문자열과 URL 해시 보존
- README의 루트 페이지 설명 최신화

# 코드 설명

## 왜 필요한가

`python -m http.server`는 디렉터리 루트 요청에서 `index.html` 또는 `index.htm`만 자동으로 탐색합니다. 실제 대시보드 파일은 `frontend/dashboard.html`에 있으므로 루트 진입 파일이 필요합니다.

## 어떤 원리인가

브라우저가 루트 `index.html`을 받으면 `new URL()`로 대시보드 주소를 생성합니다. 기존 요청의 쿼리 문자열과 해시를 새 주소에 복사한 뒤 `window.location.replace()`로 이동합니다.

`replace()`는 리다이렉트 페이지를 브라우저 방문 기록에 남기지 않아 사용자가 뒤로 가기를 눌렀을 때 다시 리다이렉트되는 현상을 줄입니다.

## 실행 흐름

1. 사용자가 `http://127.0.0.1:8765/`에 접속합니다.
2. Python 정적 서버가 루트 `index.html`을 반환합니다.
3. 브라우저가 `frontend/dashboard.html` 주소를 계산합니다.
4. JavaScript가 활성화되어 있으면 `location.replace()`로 즉시 이동합니다.
5. JavaScript가 비활성화되어 있으면 `meta refresh`가 이동을 수행합니다.
6. 두 자동 이동 방식이 모두 동작하지 않으면 사용자가 대시보드 링크를 직접 선택할 수 있습니다.

## 장점

- 기존 실행 명령어를 변경하지 않습니다.
- 대시보드 HTML을 복제하지 않아 단일 원본을 유지합니다.
- 대시보드의 CSS, JavaScript, 이미지 상대 경로가 깨지지 않습니다.
- 키보드와 보조 기술 사용자를 위한 수동 링크가 있습니다.

## 단점

- 이동 후 주소 표시줄은 `/frontend/dashboard.html`로 변경됩니다.
- 표준 정적 서버 특성상 루트 `index.html` 파일을 완전히 삭제할 수 없습니다.

## 다른 구현 방법

| 방법 | 장점 | 단점 |
|---|---|---|
| 대시보드를 루트 `index.html`로 이동 | 별도 리다이렉트가 없음 | 기존 상대 경로와 내부 링크를 대량 수정해야 함 |
| 사용자 정의 Python HTTP 핸들러 | 서버에서 직접 `/` 경로를 매핑 가능 | 실행 명령어와 유지관리 대상이 늘어남 |
| 기존 FastAPI `serve.py` 사용 | API와 정적 화면을 한 서버에서 제공 | `python -m http.server` 실행 조건과 다름 |

## 실무에서는

배포 환경에서는 웹 서버나 프레임워크 라우터에서 `/`를 대시보드에 직접 매핑합니다. 정적 호스팅에서는 현재 방식처럼 작은 진입 파일을 두거나 빌드 과정에서 대시보드를 `index.html`로 생성합니다.
...
|---|---|---|---|
| GET | `/` | 대시보드 이동용 HTML | 정적 서버 실행 실패 시 연결 불가 |
| GET | `/frontend/dashboard.html` | 메인 대시보드 HTML | 파일 누락 시 404 |
...
- 루트 `index.html`을 삭제하면 표준 `http.server`는 대시보드 대신 디렉터리 목록을 표시합니다.
- 대시보드 파일 경로가 바뀌면 `index.html`의 세 가지 링크(JavaScript, meta refresh, 수동 링크)를 함께 수정해야 합니다.
- 현재 이동 후 최종 URL은 `/frontend/dashboard.html`입니다.

# 변경 이력

2026-07-13 - 루트 URL을 메인 대시보드 진입점으로 변경

# 다음 작업

- 필요하면 사용자 정의 서버를 도입해 주소 표시줄을 `/`로 유지하는 방안 검토

# 작업 요약

## 완료한 내용

- 루트 접속 시 메인 대시보드로 자동 이동
- JavaScript 비활성 환경의 대체 이동 수단 제공
- 정적 페이지와 핵심 리소스 HTTP 응답 검증
- README와 대시보드 문서 최신화
...
기존 명령어와 프로젝트 폴더 구조를 유지하면서 실제 대시보드를 기본 화면으로 제공하기 위해 변경했습니다.

## 변경 사항

- 기존 루트 API/Mock 콘솔 제거
- 루트 진입 페이지를 `frontend/dashboard.html` 리다이렉트로 변경
- GitHub Pages 루트 설명을 대시보드 기준으로 수정

## 새롭게 배운 개념

- 정적 서버의 기본 문서 탐색 규칙
- `location.replace()`와 방문 기록의 관계
- JavaScript가 없을 때의 점진적 대체 수단

## 실무에서는

Nginx, CDN, 정적 호스팅 설정 또는 애플리케이션 라우터에서 기본 문서와 리라이트 규칙을 관리합니다. 리다이렉트 진입 파일을 사용할 때는 화면 코드를 복제하지 않고 실제 화면을 단일 원본으로 유지합니다.

## 개선 가능한 부분

- 자동 테스트에 리다이렉트 대상 문자열 검증 추가
- 배포 서버에서 `/` 내부 리라이트를 지원해 최종 URL 유지

## 다음 작업

- 실제 브라우저에서 루트 접속과 뒤로 가기 동작 최종 확인
...
- `/frontend/dashboard.html`로 자동 이동합니다.
- 대시보드 스타일과 JavaScript가 정상적으로 적용됩니다.
- 서버 응답 기준으로 `/`, 대시보드 HTML, CSS, JavaScript가 모두 HTTP 200을 반환합니다.

## 검증 방법

- 주소 표시줄이 `/frontend/dashboard.html`인지 확인합니다.
- 대시보드 카드와 사이드바가 표시되는지 확인합니다.
- 개발자 도구 Network 탭에서 `style.css`와 `dashboard.js`가 200인지 확인합니다.

## 실패 시 확인 사항

- 명령어를 프로젝트 루트에서 실행했는지 확인합니다.
- 8765 포트를 다른 프로세스가 사용 중인지 확인합니다.
- `frontend/dashboard.html`이 이동 또는 삭제되지 않았는지 확인합니다.
...
1. 표준 `python -m http.server`에서 `index.html`을 삭제하면 루트에 무엇이 표시될까요?
2. `location.assign()` 대신 `location.replace()`를 사용한 이유는 무엇일까요?
3. JavaScript 리다이렉트와 함께 `meta refresh`와 수동 링크를 제공한 이유는 무엇일까요?
...
루트 페이지의 역할이 변경되어 README의 폴더 설명과 GitHub Pages 접속 안내를 수정했습니다.

## 추천 Commit Message

`feat: 루트 접속을 메인 대시보드로 연결`

## Change Log

2026-07-13

- 루트 정적 페이지를 메인 대시보드 진입점으로 변경
- README 실행 페이지 설명 최신화

## Timestamp

2026-07-13 17:48:59 (KST)
